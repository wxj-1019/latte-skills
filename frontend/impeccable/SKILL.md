---
name: impeccable
description: "Use to audit, critique, polish, iterate on, and elevate a frontend interface's overall design quality - visual hierarchy, UX, accessibility, responsive behavior, theming, motion, micro-interactions, and live browser refinement; makes bland designs bolder and loud ones quieter. A comprehensive 23-command design workflow. For establishing an initial visual direction from scratch use frontend-design or design-taste-frontend; for pixel-level micro-details only use make-interfaces-feel-better."
version: 4.0.4
user-invocable: true
...
1. Run `node <skill-base-dir>/scripts/context.mjs` once per session (if the runtime shows this skill's loaded base directory, run `node <skill-base-dir>/scripts/context.mjs`; keep cwd at the user's project). Pass a named source file or route as `--target <path>`. It loads PRODUCT.md, DESIGN.md, the matching surface brief, and native-platform guidance when applicable; follow its directives and do not rerun it.
